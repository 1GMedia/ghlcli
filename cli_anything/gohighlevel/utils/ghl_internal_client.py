...
PLACEHOLDER_LOCATION_IDS = frozenset({"", "YOUR_LOCATION_ID", "YB8rMdFShcHGcZGW87mA"})
...
class TokenManager:
    ...
    def _refresh_firebase(self, refresh_token: str) -> Optional[str]:
        ...
        firebase_api_key = os.environ.get("GHL_FIREBASE_API_KEY", "").strip()
        if not firebase_api_key:
            print(
                "Error: GHL_FIREBASE_REFRESH_TOKEN is set but GHL_FIREBASE_API_KEY is missing.\n"
                "Set GHL_FIREBASE_API_KEY to the Firebase Web API key used by the GHL web app,\n"
                "or use GHL_FIREBASE_TOKEN for a direct short-lived Firebase ID token.",
                file=sys.stderr,
            )
            return None
...
class InternalGHLClient:
    ...
    def __init__(self, token_mgr: TokenManager, location_id: str):
        ...
        if location_id in PLACEHOLDER_LOCATION_IDS:
            print(
                "Error: GHL_LOCATION_ID must be set to the target location before "
                "using experimental internal workflow commands.\n"
                "Refusing to use a blank, placeholder, or legacy hard-coded location ID.",
                file=sys.stderr,
            )
            sys.exit(1)
