...
def _get_location_id() -> str:
    ...
    if not location_id or location_id == "YOUR_LOCATION_ID":
        print(
            "Error: GHL_LOCATION_ID environment variable is not set.\n"
            "Set it in .env or pass --location-id <location-id>.",
            file=sys.stderr,
        )
        sys.exit(1)
    return location_id
...
def delete(path: str, data: dict[str, Any] | None = None, version: str | None = None) -> dict:
    """Make a DELETE request to the GHL API."""
    url = f"{BASE_URL}{path}"
    resp = requests.delete(url, headers=_headers(version, path), json=data, timeout=30)
    resp.raise_for_status()
    try:
        return resp.json()
